...
{
    public $newMessageNotifications;
    public $messageCount;
    public $otherNotifications;
    public $otherNotificationCount;
 
    public function mount()
    {
        $this->newMessageNotifications = auth()->user()->unreadNotifications->
            where('type', NewMessageNotification::class);
        $this->messageCount = $this->newMessageNotifications->count();
        $this->otherNotifications = auth()->user()->unreadNotifications
            ->whereNotIn('type', NewMessageNotification::class);
        $this->otherNotificationCount = $this->otherNotifications->count();
    }
    protected function getListeners()
    {
        $userId = auth()->id();
        return [
            // Only this tiny badge refreshes when an event hits!
            "echo-private:App.Models.User.{$userId},.notification" => '$refresh',
        ];
    }
...
<div class="flex flex-col gap-2">
   @auth
        <a href="/chat"
            class="flex items-center gap-2  {{ $messageCount ? ' text-amber-500' : 'text-gray-700 hover:text-[#79af9d]' }}">
    ...
    @auth
        <a href="/notifications"
            class="flex items-center gap-2 {{ $otherNotificationCount ? 'text-amber-600' : 'text-gray-700 hover:text-[#79af9d]' }}">
            ...
            @if($otherNotificationCount > 0)
                <div x-data="{ count: {{ $otherNotificationCount }} }" 
     x-init="
        window.Echo.private('App.Models.User.{{ auth()->id() }}')
            .notification((notification) => {
