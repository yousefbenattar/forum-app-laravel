...
new class extends Component
{
    ...
    {
        // 1. Safe guard for guest users
        if (auth()->guest()) {
            return false;
        }

        return Report::where("user_id", auth()->id())
            ->where('report_id', $this->post->id)
            ->where("report_type", Post::class)
            ->exists();
    }
    ...
    public function reportPost()
    {
        ...
        Report::create([
            ...
            "report_type" => Post::class,
        ]);

        $this->reset('reason'); // Clear input field after reporting
    }
};
?>
            ...
            <div class="bg-white p-6 flex flex-col rounded-md max-w-md w-full mx-4" @click.outside="showPostReportModal = false">
                ...
                @else
                    ...
                    <div class="flex items-center gap-4 mt-4">
                        @if ($post->user->avatar)
                            <img src="{{ Storage::url($post->user->avatar) }}" class="h-10 w-10 rounded-full">
                        @else
                            <img src="{{ asset('images/profile.png') }}" class="h-10 w-10 rounded-full">
                        @endif
                        <p class="font-medium text-gray-800">{{ $post->user->name }}</p>
                    </div>
                    <p class="mt-2 text-gray-600 border-l-2 border-gray-300 pl-2 italic">{{ $post->title }}</p>
