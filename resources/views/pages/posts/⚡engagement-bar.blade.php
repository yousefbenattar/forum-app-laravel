...
new class extends Component {
    ...
    public function toggleLike()
    {
        ...
        // 2. Safely toggle using relationships
    ...
    public function toggleBookMark()
    {
        ...
        // 2. Safely toggle using relationships
        if ($this->post->bookmarks()->where('user_id', $userId)->exists()) {
            $this->post->bookmarks()->where('user_id', $userId)->delete();
        } else {
            ...
            <div class="flex gap-2 items-center">
                <div class="flex gap-2 items-center">
                    <form wire:submit.prevent="toggleLike">
                        <button type="submit" class="flex items-center gap-2 hover:scale-110 transition-transform">
                            @if (auth()->check() && $this->post->isLikedByAuthUser())
                    ...
                    <p class="text-sm font-bold">{{ $post->likes()->count() }}</p>
                </div>
            </div>
            <div class="flex gap-2 items-center">
            ...
            <div class="flex items-center gap-2 bg-black/10 hover:bg-black/30 text-black font-bold py-2 px-4 rounded-full">
                <form wire:submit.prevent="toggleBookMark">
                    <button type="submit" class="flex items-center gap-2">
                        @if (auth()->check() && $post->isbookmarked())
                            <p>إزالة</p>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
